...
install_init "$PWD" "StockPredictor"
...
case "$action" in
 docker) command -v docker >/dev/null 2>&1 || { echo "Docker is not installed." >&2; exit 1; }; docker info >/dev/null 2>&1 || { echo "Docker engine is not running." >&2; exit 1; };install_lock;install_require_space "$PWD" 2; docker compose up -d --build; wait_url "$api_url/health" && wait_url "$dashboard_url" || { docker compose logs; exit 1; };install_complete; echo "StockPredictor is ready at $dashboard_url"; open_url; exit 0 ;;
...
install_lock;install_require_space "$PWD" 2
[ -n "$uv" ] || uv=$(install_uv);sync=(sync --frozen);[ "$action" = repair ]&&sync+=(--reinstall);install_retry "dependency synchronization" "$uv" "${sync[@]}"
install_complete
launch_args=()
